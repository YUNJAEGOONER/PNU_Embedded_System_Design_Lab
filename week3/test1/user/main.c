...
//@TODO - 1 Set the clock, (//) ??? ??ø? ????? ??? ???? ???? ????? ?? ?????? ??????y? 
        ...
        RCC->CFGR2 |= (uint32_t)(RCC_CFGR2_PREDIV2_DIV5 | RCC_CFGR2_PLL2MUL13 | RCC_CFGR2_PREDIV1SRC_PLL2 | RCC_CFGR2_PREDIV1_DIV5); //sysclock값 설정함
//@End of TODO - 1
...
//@TODO - 2 Set the MCO port for system clock output
        ...
        RCC->CFGR |=  (uint32_t)RCC_CFGR_MCO_SYSCLK; //  #define  RCC_CFGR_MCO_SYSCLK ((uint32_t)0x04000000) /*!< System clock selected as MCO source */ output설정함 
//@End of TODO - 2
    }
    else {
...
//@TODO - 3 RCC Setting
    ...
    /* GPIO RCC Enable  */
    /* UART Tx, Rx, MCO port */ 
    RCC->APB2ENR |= ((uint32_t)RCC_APB2ENR_IOPAEN | (uint32_t)RCC_APB2ENR_USART1EN); // PA9 = tx, PA10 = rx, PA8 = MCO
    /* USART RCC Enable */
    // RCC->APB2ENR |= ??
	/* User S1 Button RCC Enable */
	RCC->APB2ENR |= (uint32_t)RCC_APB2ENR_IOPCEN; // Port C KEY(버튼열기)
}
...
//@TODO - 4 GPIO Configuration
    ...
    GPIOA->CRH |= ((uint32_t)GPIO_CRH_MODE8 | (uint32_t)GPIO_CRH_CNF8_1); // MCO PA8을 Mode를 11모드, CNF를 10모드로 설정
    /* USART Pin Configuration */
    GPIOA->CRH |= (((uint32_t)GPIO_CRH_MODE9 | (uint32_t)GPIO_CRH_CNF9_1) | (uint32_t)GPIO_CRH_CNF10_1); //rx tx 설정함
    //GPIOA->CRH |= (((uint32_t)GPIO_CRH_MODE9 | (uint32_t)GPIO_CRH_CNF9_1) | ((uint32_t)GPIO_CRH_MODE10 | (uint32_t)GPIO_CRH_CNF10_1)); //rx tx 설정함
    
    /* Reset(Clear) Port C CRH - User S1 Button */
    GPIOC->CRL &= ~(GPIO_CRL_CNF4 | GPIO_CRL_MODE4); // button reset
    /* User S1 Button Configuration */
    GPIOC->CRL |= GPIO_CRL_CNF4_1; // button input mode 설정
}
...
//@TODO - 8: Enable Tx and Rx
    USART1->CR1 |= USART_CR1_M | USART_CR1_RE | USART_CR1_TE;
...
//@TODO - 11: Calculate & configure BRR
    USART1->BRR |= (USART_BRR_DIV_Mantissa | 0xA9) | (USART_BRR_DIV_Fraction | 0x8A3D);
    //USART1->BRR = 0xA94;
