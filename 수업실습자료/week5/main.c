...
//@TODO - 4 GPIO Configuration
    ...
    GPIOA->CRH |= ((uint32_t)GPIO_CRH_MODE8_0 | (uint32_t)GPIO_CRH_CNF8_1); // MCO PA8을 Mode를 11모드, CNF를 10모드로 설정
    /* USART Pin Configuration */
    GPIOA->CRH |= (((uint32_t)GPIO_CRH_MODE9_0 | (uint32_t)GPIO_CRH_CNF9_1) | (uint32_t)GPIO_CRH_CNF10_1); //rx tx 설정함
...
//@TODO - 9: Stop bit : 1bit
    //USART1->CR2 |= (uint32_t)(0x00 << 12); // 1비트 정지 비트 설정
...
//@TODO - 11: Calculate & configure BRR
    //USART1->BRR |= (USART_BRR_DIV_Mantissa | 0x152) | (USART_BRR_DIV_Fraction | 0x8A3D);
    USART1->BRR = 0xA94;
